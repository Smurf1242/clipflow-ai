      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {plans.map((plan, index) => (
            <div
              key={index}
              className={`glass rounded-3xl p-8 flex flex-col h-full relative ${plan.popular ? 'border-2 border-violet-500 scale-[1.03]' : ''}`}
            >
              {plan.popular && (
              ...
              <div className="flex items-baseline mb-8">
                ...
                {plan.period && <span className="text-zinc-400 ml-2 text-2xl">{plan.period}</span>}
              </div>
              ...
              <button className={`w-full py-4 rounded-2xl font-semibold text-lg mt-auto transition-all ${plan.popular 
